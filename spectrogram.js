...
function testEar() {
            putUpOrDown("Up or Down?", "white");
            frec1 = 200 + Math.random() * 400;
            var factor = document.getElementById("myrange").value;
            rn = Math.random();
            if (rn < .5) {
                frec2 = (1 - factor / 100) * frec1;
                setTimeout(putUpOrDown, 3500, "Down", "yellow");
            } else {
                frec2 = (1 + factor / 100) * frec1;
                setTimeout(putUpOrDown, 3500, "Up", "lightgreen");
            }
            //setTimeout(putUpOrDown, 4500, "Up or Down?", "white");

            const synth = new Tone.Synth().toDestination();
            const now = Tone.now();
            synth.triggerAttackRelease(frec1, "4n", now);
            synth.triggerAttackRelease(frec1, "4n", now + 1);
            synth.triggerAttackRelease(frec2, "3n", now + 2);
        }
        ...
        function Tuning() {
            ...
            const synth = new Tone.Synth().toDestination();
            const now = Tone.now()
            synth.triggerAttackRelease("a4", "1n", now);
            synth.triggerAttackRelease("a4", "1n", now + 3);
            synth.triggerAttackRelease("a4", "1n", now + 6);
        }
        ...
        function Escala() {
            ...
            for (var i = 0; i < Cmajor.length; i++) {
                synth.triggerAttackRelease(Cmajor[i], "4n", now + i);
                setTimeout(putScale, i * 1000, Cmajor[i], "lightgreen");
            }

            setTimeout(putScale, 8000, "Scale", "white");
        ...
        function Note() {
            ...
            setTimeout(putNote, 3000, note, "lightgreen");

            //setTimeout(putNote, 4500, "Which Note?", "white");

            const synth = new Tone.Synth().toDestination();
            const now = Tone.now()
            synth.triggerAttackRelease(note, "4n", now)
            synth.triggerAttackRelease(note, "4n", now + 1)
            synth.triggerAttackRelease(note, "3n", now + 2)

        }

        function putUpOrDown(myString, color) {

            document.getElementById("3tonos").value = myString;
            document.getElementById("3tonos").style.background = color;
        }

        function putNote(myString, color) {

            document.getElementById("mynote").value = myString;
            document.getElementById("mynote").style.background = color;
        }
        ...
        function anyFreq() {
            ...
            const synth = new Tone.Synth().toDestination();
            const now = Tone.now()
            const anyfreq = document.getElementById("myFreq").value;
            synth.triggerAttackRelease(anyfreq, "1n", now);
            setTimeout(putAnyFreq, 2500, "Any Freq", "lightgreen");

        }

        function putAnyFreq(myString, color) {
            document.getElementById("myFreq").value = myString;
            document.getElementById("myFreq").style.background = color;
        }

        function putScale(myString, color) {

            document.getElementById("myscale").value = myString;
            document.getElementById("myscale").style.background = color;
        }
